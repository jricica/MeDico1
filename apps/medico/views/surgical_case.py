        ...
        class SurgicalCaseViewSet(viewsets.ModelViewSet):
            ...
            def create(self, request, *args, **kwargs):
                ...
                import json
                print("=" * 80)
                print("🔍 DEBUG - DATA RECIBIDA EN BACKEND:")
                print("=" * 80)
                print(f"request.data type: {type(request.data)}")
                print(f"request.data keys: {list(request.data.keys())}")

                # ✅ VERIFICAR PROCEDURES ESPECÍFICAMENTE
                if 'procedures' in request.data:
                    procedures = request.data.get('procedures')
                    print(f"\n📋 PROCEDURES ENCONTRADOS:")
                    print(f"   - Type: {type(procedures)}")
                    print(f"   - Es lista?: {isinstance(procedures, list)}")
                    if isinstance(procedures, list):
                        ...
                        print(f"   - Content (no es lista): {procedures}")
                else:
                    print("\n❌ NO HAY 'procedures' EN request.data")
                ...
                try:
                    ...
                    print("\n✅ CASO CREADO:")
                    print(f"   - ID: {case.id}")
                    print(f"   - Procedimientos en DB: {case.procedures.count()}")
                    print("=" * 80)
